...
def vrc_status_change(json_catch):
    ...
    status_url = "https://api.vrchat.cloud/api/1/users/[your-vrc-usrid]"  # Update-info
    ...
    if r.status_code != 200:
        ...
        if data['data']:
            ...
            if state == 'rest':
                ...
                start_re = re.search(r'(?<=T)[0-9]{2}:[0-9]{2}:[0-9]{2}', start)
                start_time = start_re.group()
                start_time_re = re.findall('\d+', start_time)
                start_time_hour = (int(start_time_re[0])+9) % 24  # JST-time change-your-country-time
                start_time_minute = int(start_time_re[1])
                start_time_second = int(start_time_re[2])
                start_datetime = datetime.time(start_time_hour, start_time_minute, start_time_second)
                print(start_datetime)

                # ポモドーロの終わる時刻
                pomodoro_time = datetime.datetime.combine(datetime.date.today(), start_datetime) + datetime.timedelta(minutes=25)
                ...
                start_time = start_re.group()
                start_time_re = re.findall('\d+', start_time)
                start_time_hour = (int(start_time_re[0])+9 )% 24 #JSTにする
                start_time_minute = int(start_time_re[1])
                start_time_second = int(start_time_re[2])
            ...
            if state == 'work':
                # print(json)
                json2 = json.dumps({"status": "active", "statusDescription": "Rest-休憩中"})
                print(json2)
